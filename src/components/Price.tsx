...
export default function Price({
  ...
  const premiumNew = useMemo(
    () => Number(String(data.premiumNew).replace(/[^\d.]/g, "")) || 0,
    [data.premiumNew]
  );
  const premiumOld = useMemo(
    () => Number(String(data.premiumOld).replace(/[^\d.]/g, "")) || 0,
    [data.premiumOld]
  );
      ...
      <div className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-8">
        <div className="mt-4 sm:mt-6 md:mt-8 lg:mt-12 xl:mt-20">
          <div className="bg-card relative rounded-2xl sm:rounded-3xl bg-white shadow-xl sm:shadow-2xl px-4 sm:px-6 md:px-8 lg:px-12 py-8 sm:py-12 md:py-16">
            <div className="grid items-stretch lg:gap-8 sm:gap-10 md:gap-12 lg:divide-y lg:grid-cols-2 lg:divide-x lg:divide-y-0 divide-[#297BFE] p-0 sm:p-6 md:p-8 lg:p-12 pb-2">
              ...
              <div className="pb-12 sm:pb-10 md:pb-12 text-center lg:pb-0 lg:pr-8 xl:pr-12 relative">
                ...
                <div className="flex justify-center mb-4 sm:mb-6">
                  <div>
                    <CtaButton cta={true} text={data.btnText} link={ctaLink} />
                  </div>
                </div>
              ...
              <div className="lg:block flex flex-col-reverse relative md:pt-12 lg:pl-8 xl:pl-12">
                ...
                <ul
                  role="list"
                  className="mt-6 lg:mt-0 space-y-3 sm:space-y-4 w-[90%] lg:w-full mx-auto pt-4 lg:pt-0 mb-0 lg:mb-8"
                >
                ...
                {/* Carte Premium - Only show if enabled in Sanity */}
                {data.showPremiumSection && (
                  <div className="relative flex flex-col gap-3 sm:gap-4 rounded-xl bg-white shadow-lg px-3 sm:px-4 py-4 sm:py-6 border border-[#F7F8F9]">
                    {/* Tooltip Premium */}
                    <div className="tooltip shadow absolute -top-[20px] sm:-top-3 left-1/2 -translate-x-1/2 sm:left-auto sm:translate-x-0 sm:-right-8 md:-right-12 lg:-right-[50px] sm:rotate-3 font-semibold text-xs bg-white px-2 py-1 rounded text-nowrap">
                      {" "}
            ...
            {/* Composant Faces2 - Only show if premium section is enabled */}
            {data.showPremiumSection && (
